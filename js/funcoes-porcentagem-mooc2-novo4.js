...
function resultadoObterPorcentagemConclusaoUnidade (evento){
    ...
    if(teste_boasvindas==1){
        ...
        if(evento.detail["status"]=="200"){
            ...
            if(porcentagem>99){
                porcentagem=100;
                console.log("Teste final de porcentagem")
                API.registrarPorcentagemConclusaoUnidade(id_unidade,100);
            }
            if(porcentagem==100){  
                habilitar_atividade();
            }else{
            ...
            porcentagem=(Math.ceil((100/quant_telas[unidade])));        
